...
var jsUTF8Encoding = require('@ali/utf8-encoding');
...
// Encode string as utf8 into dataview at offset
    ...
    function utf8Read(view, offset, length) {
        var string = "";
        var tmp = navigator.userAgent.match(/Chrome\/(\S+)/);
        var chromeVersion = 0;
        if(tmp && tmp[1]){
            chromeVersion = parseInt(tmp[1]);
            if(isNaN(chromeVersion)){
                chromeVersion = 0;
            }
        }
        try{
            if(typeof TextDecoder !== 'undefined' && chromeVersion > 41){
                var utf8Decoder = new TextDecoder('utf-8', {fatal: true});
            }else{
                var utf8Decoder = new jsUTF8Encoding.TextDecoder('utf-8', {fatal: true});
            }
            var bytes = view.buffer.slice(offset, offset + length);
            string = utf8Decoder.decode(bytes);
        }catch(e){
            string = view.buffer.slice(offset, offset + length);
        }
        return string;
    }
...
//   undefined  11000100  0xc4
    ...
    function Decoder(view, offset, bytes) {
    ...
    Decoder.prototype.parse = function () {
        ...
        switch (type) {
            ...
            case 0xcf:                
                var buf = this.bytes;
                this.offset ++;
                var num =  buf[this.offset ++] * 0x1000000 + (buf[this.offset++] << 16) +
                                                 (buf[this.offset ++ ] <<  8) + buf[this.offset++];
                return num * 0x100000000 +
                       buf[this.offset++] * 0x1000000 + (buf[this.offset++] << 16) +
                                                 (buf[this.offset++] <<  8) + buf[this.offset++];
        }
    ...
    function decode(bytes) {
        ...
        var view = new DataView(buffer);
        var decoder = new Decoder(view,0,bytes);
    ...
    exports.decodeFromB64 = function(b64){
